...
/*
// Thread 3 Callback Function
void *Thread3Func(void *pArg)
{
	int* p = (int*)pArg;
	int myNum = *p;
	...
	pthread_mutex_lock(&g_Mutex);
	g_Val += 3;
	printf( "Thread: %d. Global value increased the value by 2!\n", myNum);
	pthread_mutex_unlock(&g_Mutex);
	pthread_cond_signal(&g_Cond);
	return NULL;
}
...
{	
	...
	//threadNum[2] = 2;
	//pthread_create(&hThread[2], NULL, Thread3Func, &threadNum[2]);

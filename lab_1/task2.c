 ...
 *
 * Finds every prime strictly below n using cyclic Pthreads work allocation.
                           ...
                           PrimeResult *result)
{
    ...
    for (thread_index = 0; thread_index < thread_count; ++thread_index) {
        ...
        arguments[thread_index].odd_count = result->odd_count;
        arguments[thread_index].thread_id = thread_index;
        arguments[thread_index].thread_count = thread_count;
...
/*
 * Block-cyclic ownership spreads work across the full candidate range while
 * mixing divisibility patterns within each chunk. Contiguous writes also
 * reduce false sharing. Each index has one writer, so no mutex is required.
 */
static void *search_thread(void *argument)
{
    ThreadArguments *thread = argument;
    size_t block_count = thread->odd_count / PTHREAD_CHUNK_SIZE +
                         (thread->odd_count % PTHREAD_CHUNK_SIZE != 0);
    size_t block_index;
    size_t prime_count = 0;

    for (block_index = thread->thread_id; block_index < block_count;
         block_index += thread->thread_count) {
        size_t begin = block_index * PTHREAD_CHUNK_SIZE;
        size_t remaining = thread->odd_count - begin;
        size_t end = begin + ((remaining < PTHREAD_CHUNK_SIZE)
                                  ? remaining
                                  : PTHREAD_CHUNK_SIZE);
        size_t index;
        ...
        for (index = begin; index < end; ++index) {
            unsigned char prime =
                (unsigned char)is_prime_number(odd_value(index));

            thread->is_prime[index] = prime;
            prime_count += prime;
        }
    }
